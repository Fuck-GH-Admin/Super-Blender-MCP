...
The TCP server auto-starts when the addon is enabled. In 3D Viewport, press **N** to open sidebar → **Super MCP** tab, verify it shows "Server Running".
...
The MCP server and Blender addon communicate via newline-terminated JSON over TCP. The server auto-discovers Blender's port on first command (scans 9876-9890).
